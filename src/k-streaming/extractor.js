/**
 * K-Streaming Real Web Scraper for Nuvio
 */
import { fetchText, HEADERS } from './http.js';
import cheerio from 'cheerio-without-node-native';

const TMDB_API_KEY = "439c478a771f35c05022f9feabcca01c";
const DOMAIN = "https://k-streaming.com";
...
export async function extractStreams(tmdbId, mediaType, season, episode) {
    ...
    try {
        ...
        console.log("[K-Streaming] Scraping direct pour:", searchTitle, "(" + type + ")");
        ...
        try {
            ...
            console.log("[K-Streaming] Recherche inaccessible:", fetchErr.message);
        }
        ...
        if (searchHtml) {
            ...
            $(".item, .video-item, article").each((i, el) => {
            ...
            if (mediaPageUrl) {
                console.log("[K-Streaming] Page trouvée:", mediaPageUrl);
                try {
                    const pageHtml = await fetchText(mediaPageUrl, { headers: HEADERS });
                    const $$ = cheerio.load(pageHtml);

                    $$("iframe, .player-container iframe").each((i, el) => {
                    ...
                    console.log("[K-Streaming] Erreur scraping page:", pageErr.message);
                }
            }
        }
        ...
        console.error("[K-Streaming] Erreur globale:", error);
        return [];
    }
