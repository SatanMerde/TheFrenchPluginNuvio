/**
 * French Stream Real Web Scraper for Nuvio
 */
import { fetchText, HEADERS } from './http.js';
import cheerio from 'cheerio-without-node-native';

const TMDB_API_KEY = "439c478a771f35c05022f9feabcca01c";
const DOMAIN = "https://french-stream.al";
...
export async function extractStreams(tmdbId, mediaType, season, episode) {
    ...
    try {
        ...
        console.log("[French Stream] Scraping direct pour:", searchTitle, "(" + type + ")");

        // 1. Exécution de la recherche sur le site cible
        let searchHtml = "";
        const searchUrl = DOMAIN + "/?do=search&subaction=search&story=" + encodeURIComponent(searchTitle);
        
        try {
            searchHtml = await fetchText(searchUrl, { headers: HEADERS });
        } catch (fetchErr) {
            console.log("[French Stream] Recherche inaccessible:", fetchErr.message);
        }
        ...
        if (searchHtml) {
            ...
            $(".short-story, .film-pack, .mov-t").each((i, el) => {
                if (mediaPageUrl) return;
                const linkEl = $(el).find("a.film-title, h2 a, a").first();
            ...
            if (mediaPageUrl) {
                console.log("[French Stream] Page trouvée:", mediaPageUrl);
                try {
                    const pageHtml = await fetchText(mediaPageUrl, { headers: HEADERS });
                    const $$ = cheerio.load(pageHtml);

                    $$("iframe, .player-embed iframe, .f-desc iframe").each((i, el) => {
                    ...
                    console.log("[French Stream] Erreur scraping page:", pageErr.message);
                }
            }
        }
