/**
 * Vostfree Real Web Scraper for Nuvio
 */
import { fetchText, HEADERS } from './http.js';
import cheerio from 'cheerio-without-node-native';

const TMDB_API_KEY = "439c478a771f35c05022f9feabcca01c";
const DOMAIN = "https://vostfree.ws";
...
export async function extractStreams(tmdbId, mediaType, season, episode) {
    ...
    try {
        ...
        console.log("[Vostfree] Scraping direct pour:", searchTitle, "(" + type + ")");
        ...
        try {
            ...
            console.log("[Vostfree] Recherche inaccessible:", fetchErr.message);
        }
        ...
        if (searchHtml) {
            ...
            $(".search-result, .anime-item, .poster").each((i, el) => {
                if (mediaPageUrl) return;
                const linkEl = $(el).find(".title a, a").first();
            ...
            if (mediaPageUrl) {
                console.log("[Vostfree] Page trouvée:", mediaPageUrl);
                try {
                    const pageHtml = await fetchText(mediaPageUrl, { headers: HEADERS });
                    const $$ = cheerio.load(pageHtml);

                    $$(".player-box iframe, iframe").each((i, el) => {
                        ...
                        if (src && !src.includes("google") && !src.includes("analytics") && !src.includes("doubleclick")) {
                            ...
                            streams.push({
                                name: "Vostfree",
                                title: "🇫🇷 " + "VOSTFR" + " • " + hostName + " (1080p FHD)",
                                url: playerUrl,
                                quality: "1080p",
                                headers: HEADERS
                            });
                    ...
                    console.log("[Vostfree] Erreur scraping page:", pageErr.message);
                }
            }
        }
        ...
        console.error("[Vostfree] Erreur globale:", error);
        return [];
    }
