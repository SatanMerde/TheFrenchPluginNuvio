 ...
 * Generated: 2026-08-15T15:41:10.130Z
 */
...
function extractStreams(tmdbId, mediaType, season, episode) {
  return __async(this, null, function* () {
    ...
    try {
      ...
      console.log("[Darkino] Scraping direct pour:", searchTitle, "(" + type + ")");
      let searchHtml = "";
      const searchUrl = DOMAIN + "/search?q=" + encodeURIComponent(searchTitle);
      try {
        searchHtml = yield fetchText(searchUrl, { headers: HEADERS });
      } catch (fetchErr) {
        console.log("[Darkino] Recherche inaccessible:", fetchErr.message);
      }
      if (searchHtml) {
        const $ = import_cheerio_without_node_native.default.load(searchHtml);
        let mediaPageUrl = "";
        $(".media-card, .search-item, article").each((i, el) => {
          if (mediaPageUrl)
            return;
          const linkEl = $(el).find(".media-title a, h3 a, a").first();
        ...
        if (mediaPageUrl) {
          console.log("[Darkino] Page trouv\xE9e:", mediaPageUrl);
          try {
            ...
            console.log("[Darkino] Erreur scraping page:", pageErr.message);
          }
        }
      }
      if (streams.length === 0) {
      ...
      console.error("[Darkino] Erreur globale:", error);
      return [];
    }
    return streams;
  });
