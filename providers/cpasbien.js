 ...
 * Generated: 2026-08-15T15:41:10.123Z
 */
...
// src/cpasbien/extractor.js
var import_cheerio_without_node_native = __toESM(require("cheerio-without-node-native"));
var TMDB_API_KEY = "439c478a771f35c05022f9feabcca01c";
var DOMAIN = "https://cpasbien.si";
...
function extractStreams(tmdbId, mediaType, season, episode) {
  return __async(this, null, function* () {
    ...
    try {
      ...
      console.log("[Cpasbien] Scraping direct pour:", searchTitle, "(" + type + ")");
      let searchHtml = "";
      const searchUrl = DOMAIN + "/recherche/" + encodeURIComponent(searchTitle);
      try {
        searchHtml = yield fetchText(searchUrl, { headers: HEADERS });
      } catch (fetchErr) {
        console.log("[Cpasbien] Recherche inaccessible:", fetchErr.message);
      }
      if (searchHtml) {
        const $ = import_cheerio_without_node_native.default.load(searchHtml);
        let mediaPageUrl = "";
        $(".table-corps tr, .item").each((i, el) => {
          if (mediaPageUrl)
            return;
          const linkEl = $(el).find("a.titre, a").first();
        ...
        if (mediaPageUrl) {
          console.log("[Cpasbien] Page trouv\xE9e:", mediaPageUrl);
          try {
            ...
            $$("iframe, .video-box iframe").each((i, el) => {
            ...
            console.log("[Cpasbien] Erreur scraping page:", pageErr.message);
          }
        }
      }
      if (streams.length === 0) {
      ...
      console.error("[Cpasbien] Erreur globale:", error);
      return [];
    }
    return streams;
  });
...
// src/cpasbien/index.js
function getStreams(tmdbId, mediaType, season, episode) {
  return __async(this, null, function* () {
    try {
      console.log(`[Cpasbien] Request: ${mediaType} ${tmdbId}`);
      const streams = yield extractStreams(tmdbId, mediaType, season, episode);
      return streams;
    } catch (error) {
      console.error(`[Cpasbien] Error: ${error.message}`);
      return [];
    }
  });
}
