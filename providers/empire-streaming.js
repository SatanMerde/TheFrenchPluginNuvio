 ...
 * Generated: 2026-08-15T15:41:10.138Z
 */
...
// src/empire-streaming/extractor.js
var import_cheerio_without_node_native = __toESM(require("cheerio-without-node-native"));
var TMDB_API_KEY = "439c478a771f35c05022f9feabcca01c";
var DOMAIN = "https://empire-streaming.tv";
...
function extractStreams(tmdbId, mediaType, season, episode) {
  return __async(this, null, function* () {
    ...
    try {
      ...
      console.log("[Empire Streaming] Scraping direct pour:", searchTitle, "(" + type + ")");
      let searchHtml = "";
      const searchUrl = DOMAIN + "/?s=" + encodeURIComponent(searchTitle);
      try {
        searchHtml = yield fetchText(searchUrl, { headers: HEADERS });
      } catch (fetchErr) {
        console.log("[Empire Streaming] Recherche inaccessible:", fetchErr.message);
      }
      if (searchHtml) {
        ...
        if (mediaPageUrl) {
          console.log("[Empire Streaming] Page trouv\xE9e:", mediaPageUrl);
          try {
            ...
            console.log("[Empire Streaming] Erreur scraping page:", pageErr.message);
          }
        }
      }
      if (streams.length === 0) {
      ...
      console.error("[Empire Streaming] Erreur globale:", error);
      return [];
    }
    return streams;
  });
...
// src/empire-streaming/index.js
function getStreams(tmdbId, mediaType, season, episode) {
  return __async(this, null, function* () {
    try {
      console.log(`[Empire Streaming] Request: ${mediaType} ${tmdbId}`);
      const streams = yield extractStreams(tmdbId, mediaType, season, episode);
      return streams;
    } catch (error) {
      console.error(`[Empire Streaming] Error: ${error.message}`);
      return [];
    }
  });
}
