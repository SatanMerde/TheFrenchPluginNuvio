 ...
 * Generated: 2026-08-15T15:41:10.204Z
 */
...
// src/vfstreaming/extractor.js
var import_cheerio_without_node_native = __toESM(require("cheerio-without-node-native"));
var TMDB_API_KEY = "439c478a771f35c05022f9feabcca01c";
var DOMAIN = "https://vfstreaming.vip";
...
function extractStreams(tmdbId, mediaType, season, episode) {
  return __async(this, null, function* () {
    ...
    try {
      ...
      console.log("[VFStreaming] Scraping direct pour:", searchTitle, "(" + type + ")");
      let searchHtml = "";
      const searchUrl = DOMAIN + "/?s=" + encodeURIComponent(searchTitle);
      try {
        searchHtml = yield fetchText(searchUrl, { headers: HEADERS });
      } catch (fetchErr) {
        console.log("[VFStreaming] Recherche inaccessible:", fetchErr.message);
      }
      if (searchHtml) {
        ...
        if (mediaPageUrl) {
          console.log("[VFStreaming] Page trouv\xE9e:", mediaPageUrl);
          try {
            ...
            console.log("[VFStreaming] Erreur scraping page:", pageErr.message);
          }
        }
      }
      if (streams.length === 0) {
      ...
      console.error("[VFStreaming] Erreur globale:", error);
      return [];
    }
    return streams;
  });
...
// src/vfstreaming/index.js
function getStreams(tmdbId, mediaType, season, episode) {
  return __async(this, null, function* () {
    try {
      console.log(`[VFStreaming] Request: ${mediaType} ${tmdbId}`);
      const streams = yield extractStreams(tmdbId, mediaType, season, episode);
      return streams;
    } catch (error) {
      console.error(`[VFStreaming] Error: ${error.message}`);
      return [];
    }
  });
}
