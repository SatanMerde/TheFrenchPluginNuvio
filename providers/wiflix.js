 ...
 * Generated: 2026-08-15T15:41:10.229Z
 */
...
// src/wiflix/extractor.js
var import_cheerio_without_node_native = __toESM(require("cheerio-without-node-native"));
var TMDB_API_KEY = "439c478a771f35c05022f9feabcca01c";
var DOMAIN = "https://wiflix.voto";
...
function extractStreams(tmdbId, mediaType, season, episode) {
  return __async(this, null, function* () {
    ...
    try {
      ...
      console.log("[Wiflix] Scraping direct pour:", searchTitle, "(" + type + ")");
      let searchHtml = "";
      const searchUrl = DOMAIN + "/?s=" + encodeURIComponent(searchTitle);
      try {
        searchHtml = yield fetchText(searchUrl, { headers: HEADERS });
      } catch (fetchErr) {
        console.log("[Wiflix] Recherche inaccessible:", fetchErr.message);
      }
      if (searchHtml) {
        ...
        if (mediaPageUrl) {
          console.log("[Wiflix] Page trouv\xE9e:", mediaPageUrl);
          try {
            ...
            console.log("[Wiflix] Erreur scraping page:", pageErr.message);
          }
        }
      }
      if (streams.length === 0) {
      ...
      console.error("[Wiflix] Erreur globale:", error);
      return [];
    }
    return streams;
  });
...
function getStreams(tmdbId, mediaType, season, episode) {
  return __async(this, null, function* () {
    try {
      console.log(`[Wiflix] Request: ${mediaType} ${tmdbId}`);
      const streams = yield extractStreams(tmdbId, mediaType, season, episode);
      return streams;
    } catch (error) {
      console.error(`[Wiflix] Error: ${error.message}`);
      return [];
    }
  });
}
