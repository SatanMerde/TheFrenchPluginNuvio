 ...
 * Generated: 2026-08-15T15:41:10.187Z
 */
...
var DOMAIN = "https://papadustream.in";
  ...
  return __async(this, null, function* () {
    ...
    try {
      ...
      if (searchHtml) {
        ...
        if (mediaPageUrl) {
          console.log("[PapaDuStream] Page trouv\xE9e:", mediaPageUrl);
          try {
            ...
            $$("iframe, .player-content iframe").each((i, el) => {
            ...
            console.log("[PapaDuStream] Erreur scraping page:", pageErr.message);
          }
        }
      }
      if (streams.length === 0) {
      ...
      console.error("[PapaDuStream] Erreur globale:", error);
      return [];
    }
    return streams;
  });
...
function getStreams(tmdbId, mediaType, season, episode) {
  return __async(this, null, function* () {
    try {
      console.log(`[PapaDuStream] Request: ${mediaType} ${tmdbId}`);
      const streams = yield extractStreams(tmdbId, mediaType, season, episode);
      return streams;
    } catch (error) {
      console.error(`[PapaDuStream] Error: ${error.message}`);
      return [];
    }
  });
}
